setup () {(
    ...
    run_script="source .bashrc; $command_line |& tee output.txt; mail -s \"EC2 run finished\" -r $instance_id@$public_ip $email < output.txt; sudo shutdown -h; mail -s \"EC2 NOT stopped\" -r $instance_id@$public_ip $email  < output.txt"
